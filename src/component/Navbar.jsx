...
const Navbar = () => {
  ...
  return (
    <motion.nav
      ...
      animate="animate"
      className={twMerge(
        "relative max-w-[1440px] w-full overflow-hidden mx-auto sticky top-0 z-50 transition-all duration-300",
        scrolled
    ...
    >
      ...
      <div className="hidden lg:flex max-w-[1200px] mx-auto w-full py-3 sm:py-4 justify-between items-center px-4 xl:px-0">
        ...
        <div className="flex gap-3 sm:gap-4 md:gap-[24px] items-center">
              ...
              <div className="relative">
                ...
                {cartCount > 0 && (
                  <motion.span
                    ...
                    whileHover="hover"
                    className="absolute -top-2 -right-2 border-2 border-shade bg-primary-600 text-white text-[10px] sm:text-xs rounded-full h-5 w-5 sm:h-6 sm:w-6 flex items-center justify-center font-bold"
                  >
          ...
          <motion.button
            ...
            className="bg-gradient-primary cursor-pointer text-surface px-[16px] sm:px-[20px] md:px-[24px] h-[45px] sm:h-[50px] md:h-[60px] py-[12px] sm:py-[14px] md:py-[16px] max-h-fit rounded-full flex items-center justify-center text-sm sm:text-base md:text-base font-medium whitespace-nowrap shadow-md hover:shadow-lg transition-shadow duration-300"
            whileHover="hover"
            whileTap="tap"
            variants={buttonVariants}
          >
      ...
      <section className="block lg:hidden">
        <div
          className={twMerge(
            "flex sticky top-0 z-20 justify-between items-center px-3 sm:px-4 py-3 sm:py-4 transition-all duration-300",
            scrolled ? "bg-surface/95 backdrop-blur-lg" : "bg-surface",
          )}
        >
          <motion.img
            src={svgImg.beiroot}
          ...
          <div className="flex flex-row-reverse gap-3 sm:gap-4 items-center">
            ...
            <motion.button
              className="flex items-center justify-center h-full rounded-full p-1"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={() => navigate("/checkout")}
            >
              <div className="relative">
                ...
                {cartCount > 0 && (
                  <motion.span
                    ...
                    className="absolute -top-2 -right-1.5 font-bold border-2 border-shade bg-primary-600 text-white text-[10px] sm:text-xs rounded-full h-4 w-4 sm:h-5 sm:w-5 flex items-center justify-center"
                  >
                    {cartCount > 99 ? "99+" : cartCount}
                  </motion.span>
                )}
        ...
        <AnimatePresence>
          {isMobileOpen && (
            <motion.div
              ...
              className="fixed inset-0 top-[60px] lg:hidden z-40"
            >
              <div className="bg-dark h-[calc(100vh-60px)] overflow-y-auto shadow-2xl">
                <div className="flex flex-col items-start gap-4 sm:gap-6 py-4 w-full px-4">
                  <ul className="flex flex-col items-start w-full">
                    {navItems.map((item) => (
                      <motion.li
                        key={item.path}
                        variants={mobileItemVariants}
                        className="border-b border-gray-50/20 w-full"
                      >
                        <NavLink
                          to={item.path}
                          onClick={() => setIsMobileOpen(false)}
                          className={({ isActive }) =>
                            twMerge(
                              "text-surface text-[18px] sm:text-[20px] flex justify-between items-center py-4 font-semibold w-full hover:text-primary-500 transition-colors duration-300",
                              isActive && "text-primary-500",
                            )
                          }
                        >
                          {item.name}
                          <RiArrowRightWideFill className="ml-2 text-xl transition-transform duration-300 group-hover:translate-x-1" />
                        </NavLink>
                      </motion.li>
                    ))}
                  </ul>
                  ...
                  <motion.button
                    ...
                    className="bg-gradient-primary cursor-pointer text-surface px-[24px] h-[55px] sm:h-[60px] py-[14px] sm:py-[16px] w-full max-h-fit rounded-lg flex items-center justify-center text-base font-medium shadow-lg"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    Order Now
                  ...
                  <motion.button
                    ...
                    className="w-full bg-surface h-[50px] sm:h-[54px] flex items-center justify-center z-20 rounded-lg shadow-md"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    <div className="relative flex h-full w-full text-dark justify-center gap-2 font-bold items-center text-md">
                      {formatCurrency(cartTotal)}
                      <span className="relative">
                        <IoMdCart className="text-[22px] sm:text-[24px] text-dark" />
                        {cartCount > 0 && (
                          <span className="absolute -top-2 -right-1.5 font-bold border-2 border-shade bg-primary-600 text-white text-[10px] sm:text-xs rounded-full h-4 w-4 sm:h-5 sm:w-5 flex items-center justify-center">
                            {cartCount > 99 ? "99+" : cartCount}
                          </span>
                        )}
                      </span>
                  ...
                  >
                    <motion.a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="h-[40px] w-[40px] sm:h-[44px] sm:w-[44px] relative rounded-full bg-surface flex items-center justify-center shadow-md hover:shadow-lg transition-shadow"
                      whileHover={{ scale: 1.1, rotate: 5 }}
                      whileTap={{ scale: 0.95 }}
                    >
                      <FaInstagram className="text-[22px] sm:text-[26px] text-dark" />
                    </motion.a>
                    <motion.a
                      href="https://www.tiktok.com/@beiroot"
                      target="_blank"
              ...
              <div className="pointer-events-none">
                <img
                  src={svgImg.menuSvg}
                  alt="Menu"
                  className="w-full absolute -bottom-1 left-0 object-cover opacity-50"
                />
                <motion.img
                  src={svgImg.whatsappIcon}
                  alt="WhatsApp"
                  className="absolute right-3 sm:right-4 bottom-20 sm:bottom-24 z-50 w-[45px] sm:w-[50px] cursor-pointer"
                  whileHover={{ scale: 1.1, rotate: 5 }}
                  animate={{
                    y: [0, -8, 0],
                    transition: {
